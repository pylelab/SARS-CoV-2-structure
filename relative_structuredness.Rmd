---
title: "Relative Structured-ness Framework"
...
working <- median_structuredness_criterion %>% rename(`Structuredness` = `Structured-ness (51-nt)`) %>% rename(`Structuredness gmedian` = `gmedian...3`) %>% rename(`Shannon gmedian` = `gmedian...6`)
```

```{r}
rel_bpc <- numeric(length(working$Structuredness))
for (i in 1:length(working$Structuredness)) {
  rel_bpc[i] <- length(which(working$Structuredness < working$Structuredness[i])) / length(working$Structuredness)
}
